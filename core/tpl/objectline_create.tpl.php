...
global $price;
$sql = 'SELECT c.date_commande AS c_date FROM '.MAIN_DB_PREFIX.'commande_fournisseur AS c JOIN '.MAIN_DB_PREFIX.'commande_fournisseurdet AS cf ON cf.fk_commande = c.rowid'; 
$sql.= " WHERE cf.fk_product = "."'+jQuery(\"#idprod\").val()+'";
$sql.= " ORDER BY c_date DESC";
$resql_tmp = $this->db->query($sql);
if($resql_tmp){
	$obj_tmp = $this->db->fetch_object($resql_tmp);
	$price = $obj_tmp->c_date;
	$this->db->free($resql_tmp);
	print $price;
	// echo $price." <- prix";
}else {
	$price = 10;
...
$pmpr = new modPMPR($this->db);
// $pmpr->calc_PMPR(GETPOST('idprod'));
		...
		if (!empty($usemargins) && $user->hasRight('margins', 'creer')) {
			...
			$.post('<?php echo DOL_URL_ROOT; ?>/fourn/ajax/getSupplierPrices.php?bestpricefirst=1', { 'idprod': $(this).val(), 'token': token }, function(data) {
				if (data && data.length > 0)
				{
					...
					<!-- Début de la portion de code modifié -->

                    <!-- Affichage de deux options supplémentaires : PMPR en fonction du nombre de produits et le dernier prix d'achat -->
                    <!-- Il faut ajouter une condition pour que ce ne soit affiché uniquement si c'est activé dans les paramètres du module (Créer une constante globale pour ça) -->
                    
					<!-- calcul du PMPR (march po) -->
					var id_pmpr = '<?php
						$id = "'+jQuery(\"#idprod\").val()+'";
						dol_syslog("valeur du getpost : ".GETPOST('idprod'));
						// jQuery("#qty").val() pour récupérer la quantité saisie
						// $resPMPR = $pmpr->calc_PMPR($id);
                        echo $id;
                        // echo "'+jQuery(\"#idprod\").val()+'"; // Retourne le bon id
						// echo 'PMPR : 13';
                     ?>';
                    ...
                    options += '<option value="pmpr" price ="'+'PMPR'+'">'+pmpr+'</option>';

					<!-- Récupération du dernier prix d'achat -->
					var lastprice = '<?php 
						// Requête est bien écrite mais le query ne se fait pas
						$sql = 'SELECT c.date_commande AS c_date FROM '.MAIN_DB_PREFIX.'commande_fournisseur AS c JOIN '.MAIN_DB_PREFIX.'commande_fournisseurdet AS cf ON cf.fk_commande = c.rowid'; 
						$sql.= " WHERE cf.fk_product = "."'+jQuery(\"#idprod\").val()+'";
						$sql.= " ORDER BY c_date DESC";
						dol_syslog("Requete sql pour voir : ".$sql);
						$resql_tmp = $this->db->query($sql);
						if($resql_tmp){
							$obj_tmp = $this->db->fetch_object($resql_tmp);
							$price = $obj_tmp->c_date;
							$this->db->free($resql_tmp);
							echo $price." <- prix"; // Le code va bien ici mais n'affiche rien pour $price, le problème vient du fait que le jQuery n'extrait pas la valeur et reste en chaîne de caractère
						}else {
							echo "Resql : ".$resql_tmp;
						}
						
					?>';
					console.log('Dernier prix d\'achat : '+lastprice);

                    <!--options += '<option value="testtplfile" price ="'+'Dernier prix achat'+'"><?php echo "Test pour surcharge tpl"; ?></option>';-->
					
					<!-- Fin de la portion de code modifié -->
